...
// to jest przydatne do losowania wersetu:
...
export default class Window extends Component {
      ...
      <div key="window" className="window">
        ...
        <form>
          ...
          required>
            ...
            {/* lista przekładów */}
            {
              this.props.translations.map(translation =>
                <option key={translation._id} value={translation._id}>{translation.name}</option>
              )
            }
          ...
          required>

            {
              this.props.books.map(book =>
                <option key={book._id} value={book._id}>{book.name_pl}</option>
              )
            }
          ...
          required>
            ...
            {this.props.books[params.b - 1].verses[params.c] &&
                _.range(1, this.props.books[params.b - 1].verses[params.c] + 1).map(value => <option key={-value} value={-value}>{'-'}{params.c + 1}{':'}{value}</option>)
            }

          </select>

          <button type="button" className="black" onClick={() => { this.first(); }}>
            <i className="fa fa-backward"></i>
          </button>

          <button type="button" className="black" onClick={() => { this.previous(); }}>
            <i className="fa fa-step-backward"></i>
          </button>

          <button type="button" className="black" onClick={() => { this.next(); }}>
            <i className="fa fa-step-forward"></i>
          </button>

          <button type="button" className="black" onClick={() => { this.last(); }}>
            <i className="fa fa-forward"></i>
          </button>

          <button type="button" className="black" onClick={() => { this.random(); }}>
            <i className="fa fa-question"></i>
          </button>

          <button type="button" className="black" onClick={() => { this.undo(); }}>
            <i className="fa fa-undo"></i>
          </button>

          {/* cofnij */}
          <select
            onChange={() => { this.loadPrevious(); }}
            ref={"undo"}
            className="undoSelect"
            value={0}
          required>

            {
              this.state.previous.map((params, index) => <option key={index} value={index}>
                {params.t ? this.props.translations[params.t - 1].shortname + ' ' : ''}{this.props.books[params.b - 1].abbv}{' '}{params.c}{params.v ? ':' + params.v : ''}
                {params.v2 > 0 &&
                  '-' + params.v2
                }
                {params.v2 < 0 &&
                  '-' + (params.c + 1) + ':' + (-params.v2)
                }
              </option>)
            }
          ...
          <button type="button" className="black" onClick={() => {
            ...
            <i className={this.state.newline ? "fa fa-list-ol" : "fa fa-align-justify"}></i>
          </button>
        ...
        <div ref={"content"} className={"verses" + (this.state.newline ? " newline" : "")}>
  ...
  renderVerse(verse, bname = false) {
    ...
    return <span className="verse" key={verse._id}>
      ...
      {admin &&
        <form>
          <button type="button" className="green" onClick={() => { this.insertVerse(verse.t, verse.b, verse.c, verse.v, this.props.verses); }}><i className="fa fa-plus"></i></button>
          <button type="button" className="red" onClick={() => { this.removeVerse(verse.t, verse.b, verse.c, verse.v, this.props.verses); }}><i className="fa fa-remove"></i></button>
          {verse.v > 1 &&
            <button type="button" onClick={() => { this.joinVerse(verse.t, verse.b, verse.c, verse.v, verse.x, this.props.verses); }}><i className="fa fa-level-up"></i></button>
          }
          <button type="button" className="blue" onClick={() => { this.shiftbackVerse(verse.t, verse.b, verse.c, verse.v); }}><i className="fa fa-arrow-left"></i></button>
          <button type="button" className="blue" onClick={() => { this.shiftVerse(verse.t, verse.b, verse.c, verse.v); }}><i className="fa fa-arrow-right"></i></button>
        </form>
      }
      ...
      {admin &&
        <form className="editVerse">
          <textarea value={verse.x} onChange={(event) => { this.updateVerse(verse.t, verse.b, verse.c, verse.v, event.target.value); }}></textarea>
        </form>
      }
  ...
  changeVerse(translation = undefined, book = undefined, chapter = undefined, verse = undefined, verse2 = undefined) {
    ...
    if(verse == 0 && translation == 0) { verse = 1; verse2 = 0; }
   ...
   setPrevious(params) {
     this.setState({
       previous: [params, ...this.state.previous].slice(0,10)
     });
   }
   ...
   undo() {
     ...
     this.setState({
       previous: [...previous.slice(1), previous[0]]
     });
   }
